...
class NetworkResponse {
  ...
  NetworkResponse({this.statusCode, this.data, this.success = false});

  static NetworkResponse ok(String data, [int statusCode = 200]) {
    return NetworkResponse(data: data, success: true, statusCode: statusCode);
  }

  static NetworkResponse valError(String data) {
    return NetworkResponse(data: data, success: false, statusCode: 400);
  }

  static NetworkResponse authError(String data) {
    return NetworkResponse(data: data, success: false, statusCode: 401);
  }
...
class NetworkRequestMaker {
  ...
  static late int timeoutSeconds;
  ...
  static Future<void> initialize(NetworkConfig config) async {
    ...
    NetworkRequestMaker.scheme =
        config.scheme == NetworkScheme.http ? 'http' : 'https';
  ...
  Future<NetworkResponse> execute(
    ...
    if (Uri.tryParse(path!.call(identifiers))!.isAbsolute) {
      ...
      finalUrl = Uri(
          ...
          port: port,
          scheme: scheme,
          path: (path != null) ? path(identifiers) : '',
          queryParameters: query);
    ...
    try {
      response = await _client
          .send(request)
          .timeout(Duration(seconds: NetworkRequestMaker.timeoutSeconds));
    } catch (e) {
      print(e);
      return NetworkResponse.netError();
    }
